...
struct _lcm_t {
    LCMRecMutex mutex;         // guards data structures
...
static inline void lcm_rec_mutex_init(LCMRecMutex *mutex)
{
    mutex->depth = 0;
    g_rec_mutex_init(&mutex->mutex);
}

static inline void lcm_rec_mutex_clear(LCMRecMutex *mutex)
{
    mutex->depth = 0;
    g_rec_mutex_clear(&mutex->mutex);
}

static inline void lcm_rec_mutex_lock(LCMRecMutex *mutex)
{
    mutex->depth++;
    g_rec_mutex_lock(&mutex->mutex);
}

static inline void lcm_rec_mutex_unlock(LCMRecMutex *mutex)
{
    mutex->depth--;
    g_rec_mutex_unlock(&mutex->mutex);
}

static inline void lcm_rec_mutex_lock_full(LCMRecMutex *mutex, guint depth)
{
    while (depth--)
    {
        g_rec_mutex_lock (&mutex->mutex);
...
static inline guint lcm_rec_mutex_unlock_full(LCMRecMutex *mutex)
{
    gint depth;
    gint i;
...
int lcm_dispatch_handlers(lcm_t *lcm, lcm_recv_buf_t *buf, const char *channel)
{
    ...
    for (int i = 0; i < nhandlers; i++) {
        ...
        if (!h->marked_for_deletion && h->num_queued_messages > 0) {
            h->num_queued_messages--;
            int depth = lcm_rec_mutex_unlock_full(&lcm->mutex);
            h->handler(buf, channel, h->userdata);
            lcm_rec_mutex_lock_full(&lcm->mutex, depth);
        }
    }
...
{
    lcm_rec_mutex_lock(&subs->lcm->mutex);
    subs->max_num_queued_messages = num_messages;
    lcm_rec_mutex_unlock(&subs->lcm->mutex);
    return 0;
}
